...
function ExpandedTabs({ tabs, className, onChange }: ExpandedTabsProps) {
  ...
  <div
    ref={containerRef}
    className={`flex flex-row items-center gap-1 rounded-full border border-slate-200 bg-white/70 dark:bg-black dark:border-slate-700 p-1 shadow-md backdrop-blur-sm ${className || ""}`}
  >
...
const { theme, toggleTheme } = useTheme(); // ✅ must come BEFORE defining TABS
  const TABS: TabItem[] = [
  { type: "tab", title: "Home", icon: HomeIcon },
  { type: "tab", title: "Profile", icon: UserIcon },
  ...
  { type: "separator" },
  {
    ...
    render: () => (
      ...
      >
        {theme === "dark" ? <SunIcon size={24} className="w-5 h-5" /> : <MoonIcon size={24} className="w-5 h-5" />}
      </button>
    ),
  },
